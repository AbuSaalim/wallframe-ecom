...
export async function GET(request, { params }) {
  try {
    ...
    const getProductVariant = await ProductVariantModel.findOne({ _id: id, deletedAt: null })
      .populate('category', 'name _id')
      .populate('media', 'secure_url _id alt');
    console.log('📦 Found product:', getProductVariant);
    if (!getProductVariant) {
      return response(false, 404, 'Product Variant not found.');
    }
    return response(true, 200, 'Product Variant found.', getProductVariant);
  } catch (error) {
    console.error('GET /api/product-variant/get/[id] error:', error);
